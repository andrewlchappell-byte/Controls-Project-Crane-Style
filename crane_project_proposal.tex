...
\author{Caleb K. D. (calebkd34) \\ Draft prepared: \today}
\date{}
...
\section*{Memo}
To: Project Supervisor / Course Instructor \\
From: Caleb K. D. (calebkd34) \\
Date: \today \\
...
Below is a schematic (not to scale) drawn with TikZ.
...
\begin{tikzpicture}[scale=1, every node/.style={scale=0.95}]
  ...
  \draw[thick] (4,3) -- (8,3);
  % trolley
  \draw[fill=gray!20] (5.2,2.6) rectangle (6.0,3.0) node[midway]{trolley};
  % hoist cable and load
  \draw[thick] (5.6,2.6) -- (5.6,1.0);
  \draw[thick,fill=black!10] (5.0,0.2) rectangle (6.2,1.0) node[midway]{load $m$};
  % barrier
  \draw[fill=red!20] (7.2,0) rectangle (7.6,1.6) node[midway,right]{barrier $x_b,\; h_b$};
  % sensors
  \draw[->] (5.6,3.1) -- ++(0,0.6) node[above]{ultrasonic/laser for $z$};
  \draw[->] (6.0,2.8) -- ++(1.2,-0.8) node[right]{forward sensor for barrier};
  % axes & variables
  \draw[->] (2, -0.6) -- ++(3,0) node[right]{$x$};
  \draw[->] (11.5, 2.2) -- ++(0,-1.2) node[below]{$z$ (down positive)};
  % labels
  \node at (3.2,2.2) {gantry frame};
  \node at (9.2,2.2) {rail span};
\end{tikzpicture}
\end{center}
...
Proposed 10-week timeline (example for a semester project):

- Week 1: Detailed design, parts list, ordering components. Create CAD / sketches.
- Week 2: Mechanical assembly of rails, trolley, hoist; mount sensors and motors.
- Week 3: Electrical wiring, motor drivers, basic motor tests (open-loop).
- Week 4: Sensor driver integration on ESP32 (MicroPython), basic reading and filtering.
- Week 5: System identification: measure friction, motor constants, sensor characteristics.
- Week 6: Implement and tune PID controllers for $z$ (hoist) and $x$ (trolley) axes.
- Week 7: Implement barrier-detection logic and high-level state machine; conservative tests.
- Week 8: Performance tuning (speeds, clearance margin), add safety features (limits, E-stop).
- Week 9: Robustness testing, corner cases, documentation of experiments.
- Week 10: Final report (PDF), code repository (GitHub) with README and usage, demonstration video and live demo.
